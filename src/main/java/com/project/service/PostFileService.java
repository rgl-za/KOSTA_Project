...
import com.project.domain.PostFileResponseDTO;
...
public class PostFileService {
	...
	public PostFileResponseDTO findByFnum(Long fnum) throws Exception {
		return new PostFileResponseDTO(postFileRepository.findByFnum).get());
	}
	public List<Long> findByPostFnum(Long postFnum) throws Exception{
		return postFileRepository.findByPostFnum(postFnum);
	}
	public boolean uploadFile(MultipartHttpServletRequest multiRequest, Long postFnum) throws Exception {
		if (postFnum == null) throw new NullPointerException("Empty POST_FNUM.");
		...
		while (itr.hasNext()) {
			...
			if (fileSize > 0) {
				...
				if (!filePhyFolder.exists()) {
					// 부모 폴더까지 포함하여 경로에 폴더를 만든다.
					if (!filePhyFolder.mkdirs()) {
				...
				if (saveFile.isFile()) {
					...
					/* 생성한 파일 객체를 업로드 처리하지 않으면 임시파일에 
					 * 저장된 파일이 자동적으로 삭제되기 때문에 transferTo(File f)
					 * 매서드를 이용해서 업로드 처리한다. */
					mFile.transferTo(saveFile);
				}
				PostDTO postFile = PostDTO.builder()
						.postFnum(fnum)
						...
						.deleteYn("N")
						.build();
				resultList.add(postFileRepository.save(postFile).getFnum());
			}
		}
	}
