...
import com.project.domain.PostFileRequestDTO;
import com.project.domain.PostFileResponseDTO;
import com.project.service.PostFileService;
...
public class PostFileController {
	...
	public void downloadFile(@RequestParam() Long fnum, HttpServletResponse response) throws Exception {
		try {
			// 파일 정보 조회 
			PostFileResponseDTO fileInfo = postFileService.findById(fnum);
		...
		if(fSize > 0) {
			...
			String encodeFilename = "attachment; filename*=" + "UTF-8" + "''" + URLEncoder.encode(fileInfo.getOrigFileName(), "UTF-8");
			 ...
			 * int read(byte[] b. int off, int len) : 'len' 길이만큼 읽어서
			 * 배열의 'off'번째 위치부터 저장*/
...
public String updateDeleteYn(Model model, PostFileRequestDTO postFileRequestDTO) throws Exception {
	try {
		model.addAttribute("result", postFileService.updateDeleteYn(postFileRequestDTO.getFnumArr()));
	} catch (Exception e) {
		throw new Exception(e.getMessage());
	}
	return "jsonView";
